...
export const createLibraryItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!files || files.length === 0) {
    res.status(400).json({ success: false, message: "File upload is required." });
    return;
  }
  ...
  if (createdItems.length === 0) {
    res.status(400).json({ success: false, message: "At least one valid language field is required." });
    return;
  }
...
export const getLibraryItemById = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!isValidObjectId(id)) {
    res.status(400).json({ success: false, message: "Invalid library item ID." });
    return;
  }
