...
const ensureUniqueSlug = async (
  ...
  while (true) {
    const clash = await SkillModel.findOne({
      tenant,
      slug,
...
/* ---------- CREATE ---------- */
export const createSkill: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const baseSlug = slug
      ...
      : slugify(title?.[locale] || title?.en || "skill", { lower: true, strict: true });
    const uniqueSlug = await ensureUniqueSlug((req as any).tenant, baseSlug, Skill);
...
/* ---------- UPDATE ---------- */
export const updateSkill: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Skill } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  const doc = await Skill.findOne({ _id: id, tenant: (req as any).tenant });
  ...
  if (U.slug !== undefined) {
    ...
    doc.slug = await ensureUniqueSlug((req as any).tenant, base, Skill, doc._id.toString());
  }
  ...
  if (U.removedImages) {
    try {
      ...
      for (const r of removed) {
        if (r.url) {
          const localPath = path.join("uploads", (req as any).tenant, "skill-images", path.basename(r.url));
...
/* ---------- LIST (Admin) ---------- */
export const adminGetAllSkill: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Skill } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
...
/* ---------- GET by ID (Admin) ---------- */
export const adminGetSkillById: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Skill } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  const { id } = req.params;
  ...
  const doc: any = await Skill.findOne({ _id: id, tenant: (req as any).tenant })
...
/* ---------- DELETE ---------- */
export const deleteSkill: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Skill } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  for (const img of doc.images || []) {
    try {
      const localPath = path.join("uploads", (req as any).tenant, "skill-images", path.basename(img.url));
      if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
