...
export const createTeam = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const safeTeam = normalizeTeamItem(team.toObject());

      res.status(201).json({ success: true, message: t("created"), data: safeTeam });
    } catch (err: any) {
...
export const updateTeam = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (updates.title) {
      team.title = mergeLocalesForUpdate(
        team.title,
    ...
    // --- Normalization ---
    const safeTeam = normalizeTeamItem(team.toObject());
    res.status(200).json({ success: true, message: t("updated"), data: safeTeam });
  }
);
...
export const adminGetAllTeam = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const data = (teamList || []).map(normalizeTeamItem);
...
export const adminGetTeamById = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    // --- Güvenli array normalization ---
    const populated = normalizeTeamItem(team);
...
export const deleteTeam = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    for (const img of Array.isArray(team.images) ? team.images : []) {
      const localPath = path.join(
        "uploads",
        req.tenant,
        "team-images",
