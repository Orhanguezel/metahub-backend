// src/modules/payments/gateway/adapters/iyzico.adapter.ts
import type {
...
function verifyIyzico(req: Request, creds: GatewayCredentials): boolean {
  try {
    const secret = (creds as any)?.secretKey || (creds as any)?.apiKey;
    const sig = String(req.headers["x-iyzi-signature"] || "");
...
export class IyzicoAdapter implements GatewayAdapter {
  ...
  async createCheckout(input: CreateCheckoutInput): Promise<CreateCheckoutOutput> {
    ...
    return { providerRef: ref, hostedUrl: `https://iyzico.mock/pay/${ref}`, payload: { amount: _amount, currency: _currency } };
  }
  ...
  async parseWebhook(req: Request, creds: GatewayCredentials): Promise<WebhookEvent> {
    const verified = verifyIyzico(req, creds);
