...
const validateLinksOptional = body("links")
  ...
  .custom((l, { req }) => {
    ...
    if (!checkId(l.order)) throw new Error(t("validation.linksOrder"));
    return true;
  });
...
const STATUS_ALL = ["pending","confirmed","partially_allocated","allocated","failed","canceled","refunded"] as const;
const METHODS_ALL = ["cash","bank_transfer","sepa","ach","card","wallet","check","other"] as const;

/* ------------- CREATE ------------- */
export const validateCreatePayment = [
  body("kind").optional().isIn(["payment", "refund", "chargeback"]),
  body("status").optional().isIn(STATUS_ALL as unknown as string[]),
  body("method").notEmpty().isIn(METHODS_ALL as unknown as string[]),
...
export const validateUpdatePayment = [
  body("kind").optional().isIn(["payment", "refund", "chargeback"]),
  body("status").optional().isIn(STATUS_ALL as unknown as string[]),
  body("method").optional().isIn(METHODS_ALL as unknown as string[]),
  body("provider").optional().isString(),
  body("providerRef").optional().isString(),
  body("reference").optional().isString(),
...
export const validateChangePaymentStatus = [
  body("status")
    .isIn(STATUS_ALL as unknown as string[])
    .withMessage((_, { req }) =>
...
export const validatePaymentListQuery = [
  query("status").optional().isIn(STATUS_ALL as unknown as string[]),
  query("kind").optional().isIn(["payment", "refund", "chargeback"]),
  query("method").optional().isIn(METHODS_ALL as unknown as string[]),
