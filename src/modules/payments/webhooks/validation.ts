...
export const ALLOWED_EVENTS = [
  ...
  "payment.created",
  "payment.refunded",
  "refund.created",          // ← eklendi (emit edilenle uyumlu)
  "refund.updated",          // ← eklendi (emit edilenle uyumlu)
  "promotion.redeemed",
  "coupon.created",
  "coupon.updated",
  "menuitem.updated",
...
const RESERVED_HEADERS = ["content-type", "x-mh-signature", "x-mh-timestamp", "x-mh-event", "x-mh-tenant", "x-mh-delivery-id"];
...
export const validateObjectId = (field: string) => [
  param(field).isMongoId().withMessage((_, { req }) =>
    translate("invalidObjectId", req.locale || getLogLocale(), translations)
  ),
  validateRequest,
];
...
export const validateCreateEndpoint = [
  ...
  body("targetUrl").custom((v) => isURLish(v)).withMessage((_, { req }) =>
    translate("invalidUrl", req.locale || getLogLocale(), translations)
  ),
