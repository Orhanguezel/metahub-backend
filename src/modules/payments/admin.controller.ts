...
const PROVIDERS = ["stripe","paypal","iyzico","paytr","papara","paycell","craftgate","manual"] as const;

/* Sağlayıcı bazlı minimum credential seti */
...
export const createGateway = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!payload.provider || !(PROVIDERS as readonly string[]).includes(payload.provider)) {
...
export const testGateway = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const minOk = NEEDS[provider]?.(creds) ?? false;
