...
import type { IEnsotekCategory } from "./types";
...
export const createEnsotekCategory = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // 2️⃣ Images
      const images: IEnsotekCategory["images"] = [];
...
export const updateEnsotekCategory = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    // Diğer alanlar
    const directFields: (keyof IEnsotekCategory)[] = ["isActive"];
...
export const getAllEnsotekCategories = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const categories = await EnsotekCategory.find(filter).sort({
      createdAt: -1,
    });
