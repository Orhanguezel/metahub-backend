...
export const getAllSpareParts = asyncHandler(
  async (req: Request, res: Response) => {
    const { Sparepart } = await getTenantModels(req);
    const parts = await Sparepart.find({ tenant: req.tenant }).sort({
      createdAt: -1,
    });
    res.status(200).json({
      success: true,
...
export const getSparePartById = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!isValidObjectId(id)) {
      res
        .status(400)
        .json({ success: false, message: "Invalid spare part ID." });
      return;
    }
    const part = await Sparepart.findOne({ _id: id, tenant: req.tenant });
    if (!part) {
      res
        .status(404)
        .json({ success: false, message: "Spare part not found." });
  ...
  async (req: Request, res: Response) => {
    ...
    if (req.files && Array.isArray(req.files)) {
      const newImages = (req.files as Express.Multer.File[]).map(
        (file) => `${BASE_URL}/uploads/spareparts/${file.filename}`
      );
...
export const deleteSparePart = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    await Sparepart.deleteOne({ _id: id, tenant: req.tenant });
    if (!part) {
      res
        .status(404)
        .json({ success: false, message: "Spare part not found." });
