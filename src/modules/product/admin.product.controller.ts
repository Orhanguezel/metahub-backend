  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const imageUrls = images
        ? await Promise.all(
            images.map(async (file) => await optimizeImage(file.path))
          )
        : [];
...
export const updateProduct = asyncHandler(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const product = await Product.findOne({ _id: id, tenant: req.tenant });
      if (!product) {
        res.status(404).json({
      ...
      if (files && files.length > 0) {
        const newImages = await Promise.all(
          files.map(async (file) => await optimizeImage(file.path))
        );
...
export const deleteProduct = asyncHandler(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { id } = req.params;
      const { Product, Stockmovement } = await getTenantModels(req);
