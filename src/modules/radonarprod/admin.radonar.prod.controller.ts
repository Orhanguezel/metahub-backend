...
import { RadonarProd, IRadonarProd } from ".";
...
// ✅ CREATE
export const createRadonarProd = asyncHandler(async (req: Request, res: Response) => {
  let {
    name,
    description,
    tags,
  ...
  const slug = slugify(name?.en || name?.tr || name?.de || brand || "radonar-product", {
    lower: true,
    strict: true,
  });
...
// ✅ UPDATE
export const updateRadonarProd = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  const updatableFields: (keyof IRadonarProd)[] = [
    "name", "description", "tags", "category", "brand", "price", "stock", "stockThreshold",
    "frameMaterial", "brakeType", "wheelSize", "gearCount", "suspensionType", "color",
    "weightKg", "isElectric", "batteryRangeKm", "motorPowerW", "isPublished"
  ];
...
// ✅ DELETE
export const deleteRadonarProd = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// ✅ GET ALL
export const adminGetAllRadonarProd = asyncHandler(async (_req: Request, res: Response) => {
...
// ✅ GET BY ID
export const adminGetRadonarProdById = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
