...
export const getAllCarts = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (status && ["open", "ordered", "cancelled"].includes(status as string)) filter.status = status;
    ...
    res.status(200).json({ success: true, data: carts, message: cartT("cart.admin.fetchedAll", locale) });
    return;
  } catch (error) {
    logger.withReq.error(req, cartT("cart.admin.fetchError", locale) + " " + String(error));
    next(error);
  }
});

// ✅ Get single cart by ID
export const getSingleCart = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const locale = (req.locale as SupportedLocale) || "en";
  const { Cart } = await getTenantModels(req);
  try {
    const { id } = req.params;
    if (!isValidObjectId(id)) {
      ...
      res.status(400).json({ success: false, message: cartT("cart.admin.invalidId", locale) });
      return;
    }
    ...
    logger.withReq.info(req, cartT("cart.admin.fetchedSingle", locale, { id }));
    res.status(200).json({ success: true, data: cart, message: cartT("cart.admin.fetchedSingle", locale) });
    return;
  } catch (error) {
    logger.withReq.error(req, cartT("cart.admin.fetchSingleError", locale) + " " + String(error));
    next(error);
  }
});
  ...
  try {
    ...
    if (status) {
      if (!["open", "ordered", "cancelled"].includes(status)) {
        logger.withReq.warn(req, cartT("cart.admin.invalidStatus", locale, { status }));
        res.status(400).json({ success: false, message: cartT("cart.admin.invalidStatus", locale) });
        return;
      }
      cart.status = status;
    }
    ...
    res.status(200).json({ success: true, message: cartT("cart.admin.updated", locale), data: cart });
    return;
  } catch (error) {
    logger.withReq.error(req, cartT("cart.admin.updateError", locale) + " " + String(error));
    next(error);
  }
});

// ✅ Delete cart
export const deleteCart = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const locale = (req.locale as SupportedLocale) || "en";
  const { Cart } = await getTenantModels(req);
  try {
    const { id } = req.params;
    ...
    res.status(200).json({ success: true, message: cartT("cart.admin.deleted", locale) });
    return;
  } catch (error) {
    logger.withReq.error(req, cartT("cart.admin.deleteError", locale) + " " + String(error));
    next(error);
  }
});

// ✅ Toggle cart active status
export const toggleCartActiveStatus = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const locale = (req.locale as SupportedLocale) || "en";
  const { Cart } = await getTenantModels(req);
  try {
    const { id } = req.params;
    ...
    const msgKey = cart.isActive ? "cart.admin.activated" : "cart.admin.deactivated";
    logger.withReq.info(req, cartT(msgKey, locale, { id }));

    res.status(200).json({ success: true, message: cartT(msgKey, locale), isActive: cart.isActive });
    return;
  } catch (error) {
    logger.withReq.error(req, cartT("cart.admin.toggleActiveError", locale) + " " + String(error));
    next(error);
  }
});
