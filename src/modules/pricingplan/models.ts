...
const FeatureItemSchema = new Schema<IPricingPlanFeatureItem>(
  {
...
const TierSchema = new Schema<IPricingPlanTier>(
  {
...
PricingPlanSchema.index({ tenant: 1, code: 1 }, { unique: true, sparse: true });
PricingPlanSchema.index({ tenant: 1, slug: 1 }, { unique: true, sparse: true });
PricingPlanSchema.index({ tenant: 1, status: 1, isActive: 1, isPublished: 1 });
PricingPlanSchema.index({ tenant: 1, category: 1, planType: 1, order: 1 });
PricingPlanSchema.index({ tenant: 1, effectiveFrom: 1, effectiveTo: 1 });
