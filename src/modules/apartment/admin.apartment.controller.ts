...
export const createApartment = asyncHandler(async (req: Request, res: Response) => {
  let { title, description, tags, category, isPublished, publishedAt } = req.body;
...
export const adminGetAllApartments = asyncHandler(async (req: Request, res: Response) => {
...
export const adminGetApartmentById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const apartment = await Apartment.findById(id).populate("category", "title").lean();

  if (!apartment || !apartment.isActive) {
    res.status(404).json({ success: false, message: "Apartment not found or inactive." });
    return;
  }
  ...
  if (updates.removedImages) {
    try {
      ...
      for (const imgUrl of removed) {
        ...
        const match = apartment.images.find((img: any) => img.url === imgUrl && img.publicId);
...
export const deleteApartment = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const img of apartment.images) {
    const localPath = path.join("uploads", "apartment-images", path.basename(img.url));
    if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
