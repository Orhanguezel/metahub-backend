...
export const createEnsotekprod = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const nameForSlug = name?.[locale] || name?.en || brand || "ensotekprod";
    const slug = slugify(nameForSlug, { lower: true, strict: true });

    const product = await Ensotekprod.create({
      name,
      slug,
      description,
      tags,
...
export const updateEnsotekprod = asyncHandler(async (req: Request, res: Response) => {
  ...
  const product = await Ensotekprod.findOne({ _id: id, tenant: req.tenant });
  if (!product) {
  ...
  // Çok dilli alanlar
  if (updates.name !== undefined) product.name = fillAllLocales(parseIfJson(updates.name));
  if (updates.description !== undefined) product.description = fillAllLocales(parseIfJson(updates.description));
...
// ✅ GET ALL
export const adminGetAllEnsotekprod = asyncHandler(async (req: Request, res: Response) => {
  const { Ensotekprod } = await getTenantModels(req);
  const products = await Ensotekprod.find({ tenant: req.tenant })
    .populate("comments")
