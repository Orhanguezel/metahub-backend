...
//import { BookingSlotRule, BookingSlotOverride } from "@/modules/bookingslot";
...
export const createSlotRule = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const exists = await BookingSlotRule.findOne(
      appliesToAll ? { appliesToAll: true } : { dayOfWeek }
    );
    if (exists) {
      res
        .status(409)
        .json({ message: "Rule for this day (or global) already exists." });
      return;
    }
...
export const createSlotOverride = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (existing) {
      res
        .status(409)
        .json({ message: "Override already exists for this date." });
      return;
    }
...
export const getAvailableSlots = asyncHandler(async (req, res) => {
  ...
  const { BookingSlotRule, BookingSlotOverride, Booking } =
  ...
  if (!ruleDoc)
    ruleDoc = await BookingSlotRule.findOne({
      appliesToAll: true,
      isActive: true,
      tenant: req.tenant,
    });
...
export const deleteSlotRule = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
...
export const deleteSlotOverride = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
...
export const getAllSlotRules = asyncHandler(
  async (req: Request, res: Response) => {
    const { BookingSlotRule } = await getTenantModels(req);
    const rules = await BookingSlotRule.find({ tenant: req.tenant }).sort(
      "dayOfWeek"
    );
...
export const getAllSlotOverrides = asyncHandler(
  async (req: Request, res: Response) => {
    const { BookingSlotOverride } = await getTenantModels(req);
