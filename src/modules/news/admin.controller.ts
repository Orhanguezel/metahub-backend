...
const ensureUniqueSlug = async (
  ...
  while (true) {
    const clash = await NewsModel.findOne({
      tenant,
      slug,
...
/* ---------- CREATE ---------- */
export const createNews: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const baseSlug = slug
      ...
      : slugify(title?.[locale] || title?.en || "news", { lower: true, strict: true });
    const uniqueSlug = await ensureUniqueSlug((req as any).tenant, baseSlug, News);
...
/* ---------- UPDATE ---------- */
export const updateNews: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { News } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  const doc = await News.findOne({ _id: id, tenant: (req as any).tenant });
  ...
  if (U.slug !== undefined) {
    ...
    doc.slug = await ensureUniqueSlug((req as any).tenant, base, News, doc._id.toString());
  }
  ...
  if (U.removedImages) {
    try {
      ...
      for (const r of removed) {
        if (r.url) {
          const localPath = path.join("uploads", (req as any).tenant, "news-images", path.basename(r.url));
...
/* ---------- LIST (Admin) ---------- */
export const adminGetAllNews: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { News } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
...
/* ---------- GET by ID (Admin) ---------- */
export const adminGetNewsById: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { News } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  const { id } = req.params;
  ...
  const doc: any = await News.findOne({ _id: id, tenant: (req as any).tenant })
...
/* ---------- DELETE ---------- */
export const deleteNews: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { News } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  for (const img of doc.images || []) {
    try {
      const localPath = path.join("uploads", (req as any).tenant, "news-images", path.basename(img.url));
      if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
