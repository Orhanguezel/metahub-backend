...
const ensureUniqueSlug = async (
  ...
  while (true) {
    const clash = await ArticlesModel.findOne({
      tenant,
      slug,
...
/* ---------- CREATE ---------- */
export const createArticles: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const baseSlug = slug
      ...
      : slugify(title?.[locale] || title?.en || "articles", { lower: true, strict: true });
    const uniqueSlug = await ensureUniqueSlug((req as any).tenant, baseSlug, Articles);
    ...
    const doc = await Articles.create({
...
/* ---------- UPDATE ---------- */
export const updateArticles: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Articles } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  const doc = await Articles.findOne({ _id: id, tenant: (req as any).tenant });
  ...
  if (U.slug !== undefined) {
    ...
    doc.slug = await ensureUniqueSlug((req as any).tenant, base, Articles, doc._id.toString());
  }
  ...
  if (U.removedImages) {
    try {
      ...
      for (const r of removed) {
        if (r.url) {
          const localPath = path.join("uploads", (req as any).tenant, "articles-images", path.basename(r.url));
...
/* ---------- LIST (Admin) ---------- */
export const adminGetAllArticles: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Articles } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
...
/* ---------- GET by ID (Admin) ---------- */
export const adminGetArticlesById: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Articles } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  const { id } = req.params;
  ...
  const doc: any = await Articles.findOne({ _id: id, tenant: (req as any).tenant })
...
/* ---------- DELETE ---------- */
export const deleteArticles: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Articles } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  for (const img of doc.images || []) {
    try {
      const localPath = path.join("uploads", (req as any).tenant, "articles-images", path.basename(img.url));
      if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
