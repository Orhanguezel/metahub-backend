...
const ArticlesSchema = new Schema<IArticles>(
  {
    ...
    category: { type: Schema.Types.ObjectId, ref: "articlescategory", required: true },
...
ArticlesSchema.index({ tenant: 1, slug: 1 }, { unique: true });
ArticlesSchema.index({ tenant: 1, createdAt: -1 });
...
ArticlesSchema.pre("validate", function (next) {
  ...
  if (!this.slug) {
    const first =
      this.title?.tr || this.title?.en || Object.values(this.title || {})[0] || "articles";
...
export const Articles: Model<IArticles> = models.articles || model<IArticles>("articles", ArticlesSchema);
