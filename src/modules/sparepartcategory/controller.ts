...
import type { ISparepartCategory } from "./types";
...
// ✅ CREATE
export const createSparepartCategory = asyncHandler(
  async (req: Request, res: Response) => {
    const locale: SupportedLocale = req.locale || getLogLocale();
    const { SparepartCategory } = await getTenantModels(req);
    ...
    try {
      ...
      // 2️⃣ Images
      const images: ISparepartCategory["images"] = [];
...
// ✅ FINAL: updateSparepartCategory
export const updateSparepartCategory = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    const category = await SparepartCategory.findOne({
      _id: id,
      tenant: req.tenant,
    });
    if (!category) {
    ...
    // Diğer alanlar
    const directFields: (keyof ISparepartCategory)[] = ["isActive"];
...
// ✅ GET ALL
export const getAllSparepartCategories = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const categories = await SparepartCategory.find(filter).sort({
      createdAt: -1,
    });
...
// ✅ GET BY ID
export const getSparepartCategoryById = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const locale: SupportedLocale = req.locale || getLogLocale() || "en";
    const { SparepartCategory } = await getTenantModels(req);
...
// ✅ DELETE
export const deleteSparepartCategory = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const locale: SupportedLocale = req.locale || getLogLocale() || "en";
    const { SparepartCategory } = await getTenantModels(req);
