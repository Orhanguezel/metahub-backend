...
/** OWNER tespiti (user > company > customer > seller) */
function getOwner(req: Request): {
  field: "userId" | "companyId" | "customerId" | "sellerId";
  value: string;
} {
  ...
  const sellerId   = req.body.sellerId   || req.query.sellerId   || req.params.sellerId;
  ...
  if (sellerId)   return { field: "sellerId",   value: String(sellerId)   };
  if (userId)     return { field: "userId",     value: String(userId)     };

  throw new Error("Owner (customerId, companyId, sellerId veya userId) gereklidir.");
}

/** Kullanılmaması gereken owner field'ları sil */
function cleanOwnerFields(obj: any, allowed: "userId" | "companyId" | "customerId" | "sellerId") {
  const all = ["userId", "companyId", "customerId", "sellerId"];
...
export const getAddresses = asyncHandler(async (req, res) => {
  ...
  let owner: { field: "userId" | "companyId" | "customerId" | "sellerId"; value: string };
...
export const createAddress = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { Address, User, Company, Customer, Seller } = await getTenantModels(req);
  ...
  if (owner.field === "userId")     await User.findByIdAndUpdate(owner.value, { $addToSet: { addresses: newAddress._id } });
  if (owner.field === "companyId")  await Company.findByIdAndUpdate(owner.value, { $addToSet: { addresses: newAddress._id } });
  if (owner.field === "customerId") await Customer.findByIdAndUpdate(owner.value, { $addToSet: { addresses: newAddress._id } });
  if (owner.field === "sellerId")   await Seller.findByIdAndUpdate(owner.value, { $addToSet: { addresses: newAddress._id } });
...
export const updateAddress = asyncHandler(async (req, res) => {
  ...
  // güncellemede mevcut owner’ı koru
  let ownerField: "userId" | "companyId" | "customerId" | "sellerId" | null = null;
  if (address.userId)     ownerField = "userId";
  if (address.companyId)  ownerField = "companyId";
  if (address.customerId) ownerField = "customerId";
  if (address.sellerId)   ownerField = "sellerId";

  if (!ownerField) {
    res.status(400).json({ success: false, message: t("addresses.ownerRequired") }); return;
  }

  let updateData: any = { ...req.body, [ownerField]: (address as any)[ownerField] };
  updateData = cleanOwnerFields(updateData, ownerField);

  const updated = await Address.findByIdAndUpdate(id, updateData, { new: true, runValidators: true });
...
export const deleteAddress = asyncHandler(async (req, res) => {
  ...
  if (address.userId)     await User.findByIdAndUpdate(address.userId,     { $pull: { addresses: id } });
  if (address.companyId)  await Company.findByIdAndUpdate(address.companyId,{ $pull: { addresses: id } });
  if (address.customerId) await Customer.findByIdAndUpdate(address.customerId,{ $pull: { addresses: id } });
  if (address.sellerId)   await Seller.findByIdAndUpdate(address.sellerId, { $pull: { addresses: id } });

  res.status(200).json({ success: true, message: t("addresses.deleted") });
});

/** -------- BULK UPDATE (owner bazlı) -------- */
export const updateAllAddresses = asyncHandler(async (req, res) => {
  ...
  if (owner.field === "userId")     await User.findByIdAndUpdate(owner.value,    { addresses: ids });
  if (owner.field === "companyId")  await Company.findByIdAndUpdate(owner.value, { addresses: ids });
  if (owner.field === "customerId") await Customer.findByIdAndUpdate(owner.value,{ addresses: ids });
  if (owner.field === "sellerId")   await Seller.findByIdAndUpdate(owner.value,  { addresses: ids });

  res.status(200).json({ success: true, message: t("addresses.updatedAll"), data: created });
});

/** -------- KULLANICI: kendi adresleri -------- */
export const getUserAddresses = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!userId) { res.status(401).json({ success: false, message: t("addresses.ownerRequired") }); return; }

  const addresses = await Address.find({ userId, tenant: req.tenant }).sort({ createdAt: -1 }).lean();
  res.status(200).json({ success: true, message: t("addresses.fetched"), data: addresses });
});

/** -------- KULLANICI: toplu güncelle -------- */
export const updateAllUserAddresses = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).json({ success: true, message: t("addresses.updatedAll"), data: createdAddresses });
});
...
export const getCompanyAddresses = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!companyId || !isValidObjectId(companyId)) { res.status(400).json({ success: false, message: t("addresses.invalidId") }); return; }

  const addresses = await Address.find({ companyId, tenant: req.tenant }).sort({ createdAt: -1 }).lean();
  res.status(200).json({ success: true, message: t("addresses.fetched"), data: addresses });
});

/** -------- COMPANY: toplu güncelle -------- */
