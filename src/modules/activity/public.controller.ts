// src/modules/activity/public.controller.ts
...
// GET /activity
export const getAllActivity: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
...
// GET /activity/:id
export const getActivityById: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const locale: SupportedLocale = (req.locale as SupportedLocale) || getLogLocale() || "en";
  const t = (key: string) => translate(key, locale, translations);
  const { Activity } = await getTenantModels(req);
...
// GET /activity/slug/:slug
export const getActivityBySlug: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const locale: SupportedLocale = (req.locale as SupportedLocale) || getLogLocale() || "en";
  const t = (key: string) => translate(key, locale, translations);
  const { Activity } = await getTenantModels(req);
