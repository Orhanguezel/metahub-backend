...
export const adminGetAllActivity = asyncHandler(async (req: Request, res: Response) => {
...
export const adminGetActivityById = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  if (!activity || !activity.isActive) {
    res.status(404).json({ success: false, message: "Activity not found or inactive." });
    return;
  }
...
export const updateActivity = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (updates.removedImages) {
    try {
      ...
      activity.images = activity.images.filter((img: any) => !removed.includes(img.url));
      ...
      for (const imgUrl of removed) {
        ...
        const match = activity.images.find((img: any) => img.url === imgUrl && img.publicId);
...
export const deleteActivity = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const img of activity.images) {
    const localPath = path.join("uploads", "activity-images", path.basename(img.url));
    if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
