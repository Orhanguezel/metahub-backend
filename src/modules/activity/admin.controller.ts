...
const ensureUniqueSlug = async (
  ...
  while (true) {
    const clash = await ActivityModel.findOne({
      tenant,
      slug,
...
/* ---------- CREATE ---------- */
export const createActivity: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const baseSlug = slug
      ...
      : slugify(title?.[locale] || title?.en || "activity", { lower: true, strict: true });
    const uniqueSlug = await ensureUniqueSlug((req as any).tenant, baseSlug, Activity);
...
/* ---------- UPDATE ---------- */
export const updateActivity: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Activity } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  const doc = await Activity.findOne({ _id: id, tenant: (req as any).tenant });
  ...
  if (U.slug !== undefined) {
    ...
    doc.slug = await ensureUniqueSlug((req as any).tenant, base, Activity, doc._id.toString());
  }
  ...
  if (U.removedImages) {
    try {
      ...
      for (const r of removed) {
        if (r.url) {
          const localPath = path.join("uploads", (req as any).tenant, "activity-images", path.basename(r.url));
...
/* ---------- LIST (Admin) ---------- */
export const adminGetAllActivity: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Activity } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
...
/* ---------- GET by ID (Admin) ---------- */
export const adminGetActivityById: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Activity } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  const { id } = req.params;
  ...
  const doc: any = await Activity.findOne({ _id: id, tenant: (req as any).tenant })
...
/* ---------- DELETE ---------- */
export const deleteActivity: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const { Activity } = await getTenantModels(req);
  const locale: SupportedLocale = (req as any).locale || "tr";
  const t = (k: string, p?: any) => translate(k, locale, translations, p);
  ...
  for (const img of doc.images || []) {
    try {
      const localPath = path.join("uploads", (req as any).tenant, "activity-images", path.basename(img.url));
      if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
