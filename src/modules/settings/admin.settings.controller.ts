...
export const upsertSettingImage = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!req.tenant) {
    res.status(400).json({ success: false, message: t("settings.tenantRequired") });
    return;
  }
  if (!key) {
  ...
  if (settings) {
    ...
    for (const img of settings.images || []) {
      const localPath = path.join("uploads", "settings-images", path.basename(img.url));
  ...
  logger.info(t("setting.upload.success"), { ...getRequestContext(req), key });
  res.status(201).json({
    success: true,
...
export const updateSettingImage = asyncHandler(async (req: Request, res: Response) => {
  ...
  let currentImages = Array.isArray(settings.images) ? [...settings.images] : [];
  ...
  if (req.body.removedImages) {
    try {
      ...
      for (const imgUrl of removed) {
        const localPath = path.join("uploads", "settings-images", path.basename(imgUrl));
        if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
        const img = currentImages.find((img: any) => img.url === imgUrl && img.publicId);
        if (img && img.publicId) {
          await cloudinary.uploader.destroy(img.publicId).catch(() => {});
        }
      }
      currentImages = currentImages.filter((img: any) => !removed.includes(img.url));
    } catch (err) {
      logger.error(t("settings.removeImagesError"), { ...getRequestContext(req), error: err });
      res.status(400).json({ success: false, message: t("settings.removeImagesError") });
      return;
    }
  }
...
export const deleteSetting = asyncHandler(async (req, res) => {
  ...
  logger.info(t("settings.deleted"), { ...getRequestContext(req), key });
...
export const upsertSetting = asyncHandler(async (req, res) => {
  ...
  if (!key || typeof value === "undefined") {
    res.status(400).json({ success: false, message: "Key and value fields are required." });
    return;
  }
  ...
  if (settings) {
    ...
    settings.isActive = typeof isActive === "boolean" ? isActive : settings.isActive;
    await settings.save();
  } else {
    settings = await Settings.create({
      key,
  ...
  logger.info(t("setting.update.success"), { ...getRequestContext(req), key });
  res.status(200).json({
    success: true,
    message: t("setting.update.success"),
    data: settings,
...
export const getAllSettingsAdmin = asyncHandler(async (req, res) => {
  const { Settings } = await getTenantModels(req);
  const settings = await Settings.find({ tenant: req.tenant }).sort({ createdAt: -1 });
...
export const getSettingByKeyAdmin = asyncHandler(async (req, res) => {
  ...
  const settings = await Settings.findOne({ key: settingKey, tenant: req.tenant });
  if (!settings) {
    res.status(404).json({ success: false, message: t("setting.error.notFound") });
    return;
  }
