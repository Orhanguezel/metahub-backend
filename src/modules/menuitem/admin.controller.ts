...
const tlFromCode = (kind: "allergens" | "additives", code: string): Record<SupportedLocale, string> =>
  (kind === "allergens" ? (ALLERGEN_MAP as any)[code] : (ADDITIVE_MAP as any)[code]) as Record<SupportedLocale, string>;

const normKVGeneric = (raw: any, kind: "allergens" | "additives") => {
  const allowed = new Set<string>(kind === "allergens" ? (ALLERGEN_KEYS as readonly string[]) : (ADDITIVE_KEYS as readonly string[]));
  const toEntry = (code: string) => allowed.has(code) ? { key: code, value: tlFromCode(kind, code) } : undefined;
  ...
  if (Array.isArray(v)) {
    return v.map((x: any) => {
      ...
      const value = x?.value && typeof x.value === "object" ? fillAllLocales(x.value) : tlFromCode(kind, key);
      return { key, value };
    }).filter(Boolean);
  }
  ...
  if (v && typeof v === "object" && typeof v.key === "string") {
    ...
    if (keys.length > 1) return keys.map((k) => toEntry(k)).filter(Boolean);
    const only = keys[0];
    const value = v.value && typeof v.value === "object" ? fillAllLocales(v.value) : tlFromCode(kind, only);
...
const normCategories = (raw: any) => {
  ...
  const normalized = arr.map((x: any) => (typeof x === "string" ? { category: x } : x));

  return normalized
    .map((x: any) => ({
      category: Types.ObjectId.isValid(x?.category) ? x.category : undefined,
      order: (Number.isInteger(x?.order) || (typeof x?.order === "string" && Number.isFinite(Number(x.order)))) ? Number(x.order) : undefined,
...
const isChannel = (x: unknown): x is Channel => typeof x === "string" && (CHANNELS as readonly string[]).includes(x);
...
const normMoney = (v: any) => {
  ...
  const toNumber = (x: any) => (typeof x === "number" ? x : (typeof x === "string" && x.trim() !== "" ? Number(x) : NaN));
  const amount = toNumber(m?.amount);
  const rawCur = typeof m?.currency === "string" ? String(m.currency).toUpperCase() : undefined;
  const currency = rawCur && ALLOWED_CURRENCIES.has(rawCur as any) ? rawCur : "TRY";
...
const normPrices = (raw: any) => {
  ...
  return arr
    .map((p: any) => {
      const kind = typeof p?.kind === "string" && PRICE_KINDS.includes(p.kind) ? p.kind : undefined;
      ...
      const minQty = Number.isInteger(p?.minQty) && p.minQty >= 0 ? p.minQty : undefined;
      ...
      return kind && value ? { kind, value, listRef, activeFrom, activeTo, minQty, channels, outlet, note } : undefined;
    })
    .filter(Boolean);
};
...
const normVariants = (raw: any) => {
  ...
  return arr
    .map((v: any) => ({
      ...
      order: (Number.isInteger(v?.order) || (typeof v?.order === "string" && Number.isFinite(Number(v.order)))) ? Number(v.order) : undefined,
      isDefault: typeof v?.isDefault === "boolean" ? v.isDefault : undefined,
      sku: v?.sku,
      barcode: v?.barcode,
      sizeLabel: fillAllLocales(v?.sizeLabel),
      volumeMl: (typeof v?.volumeMl === "number" || (typeof v?.volumeMl === "string" && v.volumeMl !== "")) ? Number(v?.volumeMl) : undefined,
      netWeightGr: (typeof v?.netWeightGr === "number" || (typeof v?.netWeightGr === "string" && v.netWeightGr !== "")) ? Number(v?.netWeightGr) : undefined,
      prices: normPrices(v?.prices) || [],
      priceListItem: Types.ObjectId.isValid(v?.priceListItem) ? v.priceListItem : undefined,
      depositPriceListItem: Types.ObjectId.isValid(v?.depositPriceListItem) ? v.depositPriceListItem : undefined,
    }))
    .filter((v: any) => !!v.code);
};
...
const withCdnBase  = (url: string) => url.replace("/upload/", "/upload/f_auto,q_auto/");
const withCdnThumb = (url: string) => url.replace("/upload/", "/upload/c_fill,w_400,h_400,q_auto,f_auto/");
...
export const createMenuItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const {
      code, slug, name, description, categories, variants, modifierGroups,
      allergens, additives, dietary, ops, sku, barcode, taxCode, isPublished, isActive,
    } = req.body || {};
    ...
    if (!variantsArr.length) {
      res.status(400).json({ success: false, message: t("validation.variantsInvalid") });
      return;
    }
    ...
    for (const file of files) {
      const imageUrl: string | undefined =
        (file as any).path || (file as any).secure_url || (file as any).url || getImagePath(file);
      ...
      if (isRemote) {
        ...
        if (shouldProcessImage()) {
          const processed = await processImageLocal((file as any).path, (file as any).filename, path.dirname((file as any).path));
    ...
    logger.withReq.error(req, t("error.create_fail"), { ...getRequestContext(req), event: "menuitem.create", error: err?.message });
...
export const updateMenuItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  const {
    code, slug, name, description, categories, variants, modifierGroups,
    allergens, additives, dietary, ops, sku, barcode, taxCode,
    isPublished, isActive, removedImages, existingImagesOrder,
  } = req.body || {};
  ...
  if (categories !== undefined) (doc as any).categories = normCategories(categories) || [];
  if (variants !== undefined)   (doc as any).variants   = normVariants(variants)   || [];
  if (modifierGroups !== undefined) (doc as any).modifierGroups = normModifierGroups(modifierGroups) || [];

  if (allergens !== undefined) (doc as any).allergens = normAllergens(allergens) || [];
  if (additives !== undefined) (doc as any).additives = normAdditives(additives) || [];
  ...
  if (removedImages) {
    try {
      const parsed = typeof removedImages === "string" ? JSON.parse(removedImages) : removedImages;
      let urls: string[] = [];
      ...
      (doc as any).images = (doc as any).images.filter((img: any) => !toDelete.has(img.url));
      for (const url of urls) {
        const localPath = path.join("uploads", req.tenant, "menuitem-images", path.basename(url));
        if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
      }
      for (const pid of publicIds) { try { await cloudinary.uploader.destroy(pid); } catch {} }
    } catch {
      res.status(400).json({ success: false, message: t("validation.imageRemoveInvalid") });
      return;
    }
  }
  ...
  if (existingImagesOrder) {
    try {
      const orderSig: string[] = typeof existingImagesOrder === "string" ? JSON.parse(existingImagesOrder) : existingImagesOrder;
...
/* ================ LIST (admin) ================ */
export const adminGetAllMenuItem = asyncHandler(async (req: Request, res: Response) => {
  const t = tByReq(req);
  const { MenuItem } = await getTenantModels(req);

  const {
    q, isActive, isPublished, category, vegetarian, vegan, containsAlcohol,
    service, limit = "200",
  ...
  if (containsAlcohol != null) filter["dietary.containsAlcohol"] = containsAlcohol === "true";
  ...
  if (q && q.trim()) {
    ...
    filter.$or = [
      ...
      ...SUPPORTED_LOCALES.map((lng) => ({ [`variants.name.${lng}`]: { $regex: qx, $options: "i" } })),
    ];
  }
...
/* ================ GET BY ID (admin) ================ */
export const adminGetMenuItemById = asyncHandler(async (req: Request, res: Response) => {
  const t = tByReq(req);
  const { MenuItem } = await getTenantModels(req);
  const { id } = req.params;
...
export const deleteMenuItem = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const img of (doc as any).images || []) {
    const localPath = path.join("uploads", req.tenant, "menuitem-images", path.basename(img.url));
    if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
