...
import translations from "@/modules/orders/i18n";
...
function generateHtml(params: OrderConfirmationParams & { locale: SupportedLocale }): string {
  ...
  const content = `
    ...
    <table style="margin-top: 20px; border-collapse: collapse;">
      ...
      <tr><td><strong>${tL("labelTotal")}:</strong></td><td>${currency}${totalPrice.toFixed(2)}</td></tr>
      <tr><td><strong>${tL("labelDiscount")}:</strong></td><td>-€${discount.toFixed(2)}</td></tr>
      <tr><td><strong>${tL("labelCoupon")}:</strong></td><td>${couponCode}</td></tr>
      <tr><td><strong>${tL("labelFinalTotal")}:</strong></td><td>${currency}${(finalTotal ?? totalPrice).toFixed(2)}</td></tr>
      <tr><td><strong>${tL("labelPaymentMethod")}:</strong></td><td>${paymentMethod}</td></tr>
      <tr><td><strong>${tL("labelPaymentStatus")}:</strong></td><td>${paymentStatus}</td></tr>
    </table>
